...
- **핵심 개념:** 변화의 원인이 **시간($t$)** 단 하나인 경우
    
- **그래픽스에서의 역할:** 특정 물체(점, 공, 캐릭터)가 시간이 지남에 따라 어디로 움직일지 결정
    
- **비유:** 날아가는 화살의 궤적 계산
    
- **수학적 특징:** $y' = f(t, y)$와 같이 오직 하나의 독립 변수($t$)에 대한 도함수만 포함

**편미분방정식 (PDEs)**
>시간과 공간이 얽힌 전체의 흐름

- **핵심 개념:** 변화의 원인이 **시간($t$)**뿐만 아니라 **공간(위치 $x, y, z$)** 등 여러 개인 경우
    
- **그래픽스에서의 역할:** 물, 연기, 옷감, 빛처럼 공간 전체가 서로 영향을 주고받으며 변하는 현상을 묘사
    
- **비유:** 호수에 돌을 던졌을 때 퍼져나가는 물결(옆 칸의 높이가 내 칸의 높이에 영향을 줌)
    
- **수학적 특징:** $\frac{\partial u}{\partial t} = \alpha \frac{\partial^2 u}{\partial x^2}$와 같이 여러 변수에 대한 편도함수들이 얽혀 있음