...
- **z** 는 이전층의 모든 입력이 각각의 가중치와 곱해진 값들이 모두 더해진 가중합이다.
	아직 활성함수를 거치지 않은 상태.
- **z** 에 [[활성함수]]**σ**를 씌우면 새로운 [[잠재벡터]]인 H(각 뉴런의 출력 값)를 뱉게 된다.

![[첨부 파일 소스/이미지 파일 소스/image/Pasted image 20230916193645.png]]
- x를z로 연결하는 W(1) 가중치 행렬과 활성함수를 곱한 [[잠재벡터]] H를 O라는 출력벡터로 연결하는 W(2) 가중치 행렬.
- 총 가중치 행렬을 두개 (W1 , W2)사용하므로 2층 신경망.

==**MLP(Multi-Layer Perceptron) 다층 퍼셉트론**==
- [[신경망(neural network)]]을 여러층으로 합성한 함수
...
- 각 [[Parameter]]를 미분해서 경사하강법으로 가중치를 업데이트.

![[첨부 파일 소스/이미지 파일 소스/image/Pasted image 20230916200028.png]]
- 위에서 부터 [[연쇄법칙]]을 통해 기울기를 구하고 밑에층으로 전달하면서 가중치를 업데이트.

![[첨부 파일 소스/이미지 파일 소스/image/Pasted image 20230916204548.png]]
[[연쇄법칙]]을 통해 [[합성함수]]를 계산
각 뉴런(노드)를 [[텐서]]라 칭함
z를 x로 [[미분]]했을 때 2(x+y)의 값이 나옴
[[역전파 알고리즘]]은 각 미분값을 메모리에 저장해줘야 되기 때문에 메모리를 더 많이 사용
...
- 기계학습에서 사용되는 [[손실함수]](Loss function)들의 작동 원리는 데이터 공간을 통계적으로 해석해서 유도한다. 예측이 틀릴 위험(risk)을 최소화하도록 데이터를 학습하는 원리는 기계학습의 기본 원리

- 회귀 분석에서 [[손실함수]]로 사용되는 L2-노름은 예측오차의 분산을 가장 최소화하는 방향으로 학습하도록 유도
...
- 확률변수는 [[확률분포]] D에 따라 이산형과 연속형 확률변수로 구분
...
- [[결합분포]]와 D가 각각 이산형인지 연속형인지는 서로 다를 수 있다.
...
- 일반적인 방법으로 f(x)의 [[적분]]을 구하는 건 불가능하다.
- 몬테카를로 방법을 통해 원하고자 하는 [[적분]]값의 1/2값을 구할 수 있다.
![[첨부 파일 소스/이미지 파일 소스/image/Pasted image 20230917020841.png]]
- 균등분포(uniform)에서 [[데이터 샘플링]]해준 다음에, 함수에 값을 대입해주고 대입된 함수값의 데이터를 산술평균 해준 후에 구간의 길이인 2만큼 곱해줌으로써 함수의 [[적분]]값을 구할 수 있다.
- 균등분포에서 [[데이터 샘플링]] -> 함수에 값 대입 -> 합수값 산술평균 -> 구간길이 2만큼 곱해주기 = 1.49387+-0.0039
...
- [[로그함수]]의 성질을 이용해서 확률분포 함수들의 곱셈을 덧셈으로 최적화 해줌.
- L 은 [[가능도]](likelihood)를 의미.
...
- 뉴럴 네트워크를 사용하는데 많은 [[Hyper Parameter]](내가 정하는 값들. 러닝 레이트,네트워크 크기 등)를 사용한다. 교차 검증을 통해 최적의 하이퍼 파라미터 셋을 찾고, 하이퍼 파라미터를 고정한 상태에서 학습시킬 때는 모든 데이터를 전부 사용.
...
![[Pasted image 20230918175119.png]]
