# **visionOS View 계층 — X 버튼, onDisappear 미동작, ScenePhase 처리까지**


Vision Pro 앱을 개발하며 당황했던 지점 중 하나는 **윈도우(View)가 사라지는 시점이 명확하게 감지되지 않는다**는 점이었다.

특히 SwiftUI에서 흔히 사용하는 onDisappear가 visionOS에서는 기대한 대로 동작하지 않는다. 그렇다면 왜 이런 현상이 발생할까?

이 글에서는 실제 개발 경험을 기반으로 **visionOS의 View 계층과 Window 동작 방식**을 정리하고,
결국 어떤 방식으로 **윈도우 닫힘(X 버튼)** 과 **디지털 크라운 버튼**을 감지하고 **앱 상태 전환을 처리할 수 있었는지** 설명하려고 한다.
...
개인적으로 사용자의 관점에서 봤을 때 'X' 버튼을 클릭했는데 최소화가 된다는 사실이 어색하게 느껴졌기에, Apple에서 왜 이렇게 설계했을까 많은 고민을 하였고 그 결과 아래와 같은 추측이 들었다.
...
이는 지극히 개발자 중심의 사고이고 일반적인 사용자는 메모리 관리가 어떻게되고 불필요한 리소스를 낭비하는지 신경쓰지 않을 가능성이 높고, Apple도 그걸 원한 것 같다.

단순히 내 눈 앞에서 사라지길 원할 때 창을 닫을 것이다.
(그래서 윈도우를 닫는 함수명도 dismisswindow로 만든 것이 아닐까..)

실제 사용자가 원하는 시야에서 종료라는 효과는 제공하되, background로 전환함으로써 기술적/UX적 이득을 챙길 수 있다.

방 안이 Vision Pro의 공간이고, TV가 하나의 window라고 생각해보자.
TV의 화면이 신경쓰여서 TV를 끈다고 해서 TV의 전원이 종료되라는 의도가 담기지 않았을 것이다.
