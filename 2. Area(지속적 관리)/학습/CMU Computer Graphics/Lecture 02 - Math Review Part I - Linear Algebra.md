...
극 좌표에 대한 설명이 강의에는 포함되어 있지 않아 아주 간단하게 살펴보자.

기존 데카르트 좌표계에서 (x, y)로 표현한다면,
극 좌표계에서는 (r, $\theta$)로 표현한다.
여기서 r은 원점에서 기준축 방향으로 떨어진 거리이고,
$\theta$는 기준축에서 반시계 방향으로 잰 각이라고 생각하면 된다.

![[Pasted image 20260122164358.png]]

위 그림에서와 같이 $\overrightarrow{OP}| = r$과  $\overrightarrow{OP}$이 극축과 이루는 각 $\theta$의 순서쌍 $(r,\theta)$으로 나타내고,
이를 극좌표라고 한다.
