...
**바닥에 이미지 추가**
1. 버튼 생성
2. 바닥 중앙에 attachments
3. 클릭하면 이미지 추가 ui 열리도록
4. 원하는 이미지 추가
	1. 해당 이미지 library에 등록됨
	2. 해당 이미지의 AssetID를 floorImage 변수에 등록함.
5. 이미지를 통해 material 생성
6. 바닥에 이미지 적용
7. 씬 업데이트

**추가된 바닥 이미지 적용**
1. 바닥을 생성할 때, floorImage변수를 확인해서 추가된 이미지가 있는지 확인함.
	1. 추가된 이미지가 있으면 해당 이미지를 통해서 material 생성해서 바닥에 적용
	2. 추가된 이미지가 없으면 기본 material 생성해서 바닥에 적용.
