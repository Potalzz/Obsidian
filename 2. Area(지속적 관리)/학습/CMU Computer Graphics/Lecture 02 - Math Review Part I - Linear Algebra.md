...
벡터에 대한 정의와 기본적인 연산은 이전에 포스팅에서 정리했었지만,
...
그들은 정규 직교 기저를 갖고 있나 ?
앞선 과정을 통해 우리는 정규 직교 기저를 보유하고있는지 판별할 수 있다.

앞선 과정을 통해, 함수들의 집합이 정규 직교인지 여부를 판별할 수 있다.

**함수의 내적**
$$\langle f, g \rangle = \int_a^b f(x)\,g(x)\,dx$$

위 함수의 내적 수식을 통해 두 함수의 곱을 전체 구간에서 적분했을 때 0이면 직교하고, 자신의 길이가 1이면 두 함수는 정규 직교(orthonormal)한다.
