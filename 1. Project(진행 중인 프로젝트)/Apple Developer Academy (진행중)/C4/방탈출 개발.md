...
**해결 방안 2. RealityKit + Entity + UIComponent방식**
RealityKit 3부터 SwiftUI View를 **HUD처럼 3D 공간에 정확하게 붙일 수 있도록** `UIComponent`가 도입됨.
해당 방식을 사용하면 `hover effect`, 포커스, 시점, 거리 등에 따라 **정확한 hit test / hover / interaction 반응**이 가능하다.

**🔧 구성 요약**
- `ModelEntity` or `Entity`를 생성하고
    
- SwiftUI View를 `UIComponent`로 등록
    
- RealityView 내에서 정확한 위치와 크기로 배치

전체 화면을 `UIComponent`로 작성하는 방식과 필요한 부분만 `UIComponent`로 작성하는 두 가지 방법이 존재한다. 지금의 경우에는 나머지 화면은 단순한 2D 화면이여도 큰 문제가 없으므로, 상호작용이 필요한 부분만 `UIComponent`로 분리해서 제작해서 `Entity`로 배치하는 방식이 적합해보인다.
