...
특히 앱의 특성상 volume과 immersive간에 상태가 동기화되어야 하기 때문에, 생성되는 모든 Entity를 하나의 rootEntity에 넣어서 관리했다. 때문에 volume에서 resize할 때 크기를 일괄적으로 조절하기가 수월했다. 그리고 Immersive이동시 rootEntity의 사이즈를 키워 현실과 같은 크기감을 느끼게 하도록 신경썼다.
Volume Resize 기능을 구현하는 데에는 아래 사이트의 자료를 많이 참고하였다.
...
프로젝트를 진행하면서 영화 업계에 종사한 팀원들과의 경험과, 현업에 종사중인 감독과 인터뷰를 진행하며 기능을 하나씩 정제해 나갔으며, visionOS의 기술을 영화 도메인에 적용해보는 과정 자체가 매우 뜻깊은 경험이었습니다. 특히 다양한 사용자 테스트를 거치면서 익숙한 사용자와 처음 사용하는 사용자 간의 경험 차이를 확인하고, QA를 진행하며 가이드 제공과 UX 개선을 반복했던 점이 중요한 배움으로 남았습니다.
