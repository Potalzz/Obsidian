...
Vision Pro 앱을 개발하며 가장 당황했던 지점 중 하나는 **윈도우(View)가 사라지는 시점이 명확하게 감지되지 않는다**는 점이었다.
...
결국 어떤 방식으로 **윈도우 닫힘(X 버튼)** 과 디지털 크라운 버튼을 감지하고 **앱 상태 전환을 처리할 수 있었는지** 설명한다.
...
**⭕ Window가 백그라운드로 이동(숨김) → 메모리는 유지 → onDisappear 미호출**

사용자가 “닫은” 것처럼 보이는 앱도 실제로는 완전히 종료되지 않고 **Background Scene**으로 전환될 뿐이다.

---

# **왜 onDisappear가 호출되지 않는가?**

SwiftUI에서 onDisappear는 **뷰가 고유한 부모 뷰 계층에서 제거될 때만** 호출된다.
하지만 visionOS에서 X 버튼을 누르면 아래 흐름으로 동작한다.
...
    /// - **Parameter** action: 씬이 활성화될 때 실행할 클로저
    /// - **Returns**: `ScenePhase` 변경을 감지하는 수정자가 적용된 뷰
...
    /// - **Parameter** action: 씬이 백그라운드로 전환될 때 실행할 클로저
    /// - **Returns**: `ScenePhase` 변경을 감지하는 수정자가 적용된 뷰
